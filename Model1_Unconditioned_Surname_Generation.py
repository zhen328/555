...
try:
    for epoch_index in range(args.num_epochs):
        ...
        # 生成示例
        model = model.cpu()
        sampled_surnames = decode_samples(sample_from_model(model, vectorizer, num_samples=2), vectorizer)
...
# 加载最佳模型参数
model.load_state_dict(torch.load(args.model_state_file, map_location='cpu'))
model = model.cpu()

# 生成10个姓氏
num_names = 10
sampled_indices = sample_from_model(model, vectorizer, num_samples=num_names)
sampled_surnames = decode_samples(sampled_indices, vectorizer)

print("-" * 15)
for name in sampled_surnames:
    print(name)