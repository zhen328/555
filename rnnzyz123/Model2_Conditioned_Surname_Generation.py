...
model = model.cpu()  # 如果之前模型在GPU上
target_language = "French"  # 替换为你的目标语言
target_index = vectorizer.nationality_vocab.lookup_token(target_language)

print(f"根据学号尾数生成 5 个 {target_language} 风格的姓氏：")
sampled_indices = sample_from_model(
    model,
    vectorizer,
    nationalities=[target_index] * 5,
    temperature=0.7
)
